...
export default class Repository {
     ...
     * @return {Promise<any>}
     */
    addUpdateStateNode(filter, node, options = { upsert: true, returnOriginal: false}){
        return new Promise((resolve, reject) => {
            this.dbService.db.collection('regionPath')
                .findOneAndReplace(
                    ...
                    let { updatedExisting, upserted } = result.lastErrorObject;
                    resolve({updated: updatedExisting, upsertedID: upserted})
                })
    ...
    async updateZipCodeArrayForLevelCollectionCollectionsConsumer(query, levelCollectionToUpdate, newZipCodeArray, options={returnOriginal: false}){
        return await this.dbService.db.collection('regionPath')
            .findOneAndUpdate(
                query,
                {
                    $set : {
                        [`${levelCollectionToUpdate}.collections.$.zipCodes`] : newZipCodeArray
                    }
                },
                options
            )
